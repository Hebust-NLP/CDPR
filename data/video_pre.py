...
def get_v_data(args, data_args):
    
    logger = logging.getLogger(args.logger_name)

    video_feats_path = data_args['video_feats_path']

    if not os.path.exists(video_feats_path):
        raise Exception('Error: The directory of features is empty.')    

    logger.info('Load Video Features Begin...')

    video_feats = load_feats(data_args, video_feats_path)
    data = padding_feats(args, video_feats)
    
    logger.info('Load Video Features Finished...')
...
def padding(feat, video_max_lengths, padding_mode = 'zero', padding_loc = 'end'):
    """
    ...
    length = feat.shape[0]
    if length > video_max_lengths:
        return feat[:video_max_lengths, :]

    if padding_mode == 'zero':
        pad = np.zeros([video_max_lengths - length, feat.shape[-1]])
    elif padding_mode == 'normal':
        mean, std = feat.mean(), feat.std()
        pad = np.random.normal(mean, std, (video_max_lengths - length, feat.shape[1]))
...
def padding_feats(args, video_feats):
    
    video_max_lengths = args.video_seq_len

    p_feats = {}

    for dataset_type in video_feats.keys():
        f = video_feats[dataset_type]
        ...
        for x in f:
            ...
            p_feat = padding(x_f, video_max_lengths)
